...
int main(){
    ...
    std::vector <std::vector< rtest::RegExp::RegSubMatch > > output1;
    regExp.reg_match_general(rawStr, -1, output1);
    cout << output1.size() << endl;
    cout << "reg_match_general" << endl;
    std::vector < std::vector < rtest::RegExp::RegSubMatch > > ::const_iterator
    iter21;
    for (iter21 = output1.begin(); iter21 != output1.end(); iter21++){
        cout << iter21->size() << endl;
        std::vector< rtest::RegExp::RegSubMatch >::const_iterator iter22;
    ...
    rtest::RegExp regExp2("^[\\d]{4}-[\\d]+-[\\d]{2}.*$");
    ...
    cout << "\n\nreg_matchR" << endl;
    string matchStrR = "/abcd/333333/8888";
    rtest::RegExp regExp2R("^\/abcd\/(\\d+)\/(\\w+)$");
    int matchRetR = regExp2R.reg_match(matchStrR);
    cout << "matchRetR=" << matchRetR << endl;


    cout << "reg_replace" << endl;
    string splitStr44 = "Showing 11 changed files  with 246 additions and 148 deletions";
    rtest::RegExp regExp44("(\\s+an[a-z]*?)\\s");
    ...
    rtest::RegExp regExp99(localePattern);
