...
int main(){
    ...
    size_t fsz = cpputils::SomeUtils::fileSize(f);
    size_t upfsz = cpputils::SomeUtils::fileSize(upf);
    cout << "filesize=" << upfsz << endl;
    char *fileContent = (char *) malloc(sizeof(char) * (fsz + 1));
    bzero(fileContent, sizeof(char) * (fsz + 1));
    cpputils::SomeUtils::getFileContent(f, fileContent);
    cpputils::httpRequest req(fileContent, fsz);
    req.printReq();

    //生成响应信息
    cpputils::httpResponse rsp(req);
